![Alt text](/img/histQtdTrips_Dublin.png?raw=true "Histograma de tamanho de trajetórias")
______________________________
![Alt text](/img/histTempoTrips_Dublin.png?raw=true "Histograma de tamanho de trajetórias")
______________________________
![Alt text](/img/histVelTrips_Dublin.png?raw=true "Histograma de tamanho de trajetórias")
______________________________
![Alt text](/img/histTempos_Dublin.png?raw=true "CDF Tempo entre pontos(Dublin)")
______________________________
![Alt text](/img/qtdViagens_Dublin.png?raw=true "CDF Distância entre pontos(Dublin)")
______________________________
![Alt text](/img/mapaCalorDestino_Dublin.png?raw=true "CDF Distância entre pontos(Dublin)")
______________________________
![Alt text](/img/mapaCalorOrigem_Dublin.png?raw=true "CDF Distância entre pontos(Dublin)")
______________________________
![Alt text](/img/mapaCalorDestino_Dublin2.png?raw=true "CDF Distância entre pontos(Dublin)")
______________________________
![Alt text](/img/mapaCalorOrigem_Dublin2.png?raw=true "CDF Distância entre pontos(Dublin)")
______________________________
![Alt text](/img/mapaCalorDestino_Dublin3.png?raw=true "CDF Distância entre pontos(Dublin)")
______________________________
![Alt text](/img/mapaCalorOrigem_Dublin3.png?raw=true "CDF Distância entre pontos(Dublin)")
______________________________
![Alt text](/img/cdfDistancia_Dublin.png?raw=true "CDF Distância entre pontos(Dublin)")
______________________________
![Alt text](/img/cdfTempo_Dublin.png?raw=true "CDF Tempo entre pontos(Dublin)")

